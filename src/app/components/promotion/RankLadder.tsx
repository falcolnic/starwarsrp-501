...
export function RankLadder({ ranks, currentRankName, nextRank }: RankLadderProps) {
    const [showFullLadder, setShowFullLadder] = useState(false);
    const currentIdx = ranks.findIndex((r) => r.name === currentRankName);

    const windowSize = 5;
    const halfWindow = Math.floor(windowSize / 2);

    let startIdx = Math.max(0, currentIdx - halfWindow);
    let endIdx = Math.min(ranks.length, startIdx + windowSize);

    if (endIdx - startIdx < windowSize) {
        startIdx = Math.max(0, endIdx - windowSize);
    }

    const focusedRanks = ranks.slice(startIdx, endIdx);
    const careerPct = ranks.length > 0 ? ((currentIdx + 1) / ranks.length) * 100 : 0;
    ...
    return (
        ...
        <div className="bg-[#0c1424]/50 border border-[var(--border)]/30 rounded p-3 space-y-2">
            ...
            <span className="text-slate-400">
                ТЕКУЩЕЕ ЗВАНИЕ: <strong className="text-[#F5C518] uppercase">{currentRankName}</strong>
            </span>
            <span className="text-slate-400 hidden sm:inline">
                ЦЕЛЬ: <strong className="text-[var(--primary)] uppercase">{nextRank?.name ?? "МАКСИМУМ"}</strong>
            </span>
            </div>
            ...
            <span>{ranks[0]?.name ?? "Рядовой-рекрут"} (Звание 01)</span>
            ...
            <span>{ranks[ranks.length - 1]?.name ?? "Клон Маршал"} (Звание {ranks.length})</span>
            </div>
        </div>
        ...
        {!showFullLadder && (
            ...
            <div className="relative flex items-center justify-between">
                {focusedRanks.map((rankObj, relativeIdx) => {
                const originalIdx = startIdx + relativeIdx;
                const isPast = originalIdx < currentIdx;
                const isCurrent = originalIdx === currentIdx;
                const isNext = nextRank?.id === rankObj.id;

                return (
                    <div key={rankObj.id} className="flex flex-col items-center flex-1 relative">
