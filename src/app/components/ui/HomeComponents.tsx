...
export function HudButton({ icon, label, primary = false }: { icon: ReactNode; label: string; primary?: boolean }) {
    return (
        <div
        className={`flex items-center gap-[10px] py-[11px] px-[28px] text-[var(--foreground)] font-[var(--font-display)] text-base tracking-[0.12em] transition-all duration-200 cursor-pointer
            ${primary 
...
export function AccordionTab({ tab, index }: { tab: { id: string; label: string; icon: ReactNode; content: ReactNode }; index: number }) {
    ...
    return (
        ...
        <button onClick={() => setOpen(o => !o)}
            className="w-full flex items-center gap-3 padding-0 px-[20px] py-[15px] bg-transparent border-none cursor-pointer text-left relative overflow-hidden"
        >
            ...
            <span className={`font-[var(--font-display)] text-sm tracking-[0.1em] flex-1 transition-colors duration-200 ${open ? "text-[var(--foreground)]" : "text-[var(--muted-foreground)]"}`}>{tab.label}</span>
...
export function PlaceholderText({ lines = 4 }: { lines?: number }) {
    const sentences = [
        "501-й Элитный Штурмовой Легион — один из ведущих подразделений быстрого реагирования в составе Великой Армии Республики, действующим на нескольких театрах военных действий.",
