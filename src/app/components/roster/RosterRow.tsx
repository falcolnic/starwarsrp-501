...
export function RosterRow({ soldier, index, onClick }: RosterRowProps) {
    ...
    const npzValue = `${soldier.cid} | ${callsign} | ${soldier.rank ?? "—"}`;
    const [copiedType, setCopiedType] = useState<"steam" | "discord" | "npz" | null>(null);

    const steamId = soldier.steamId;
    const discordId = soldier.discordId;

    const handleCopy = (e: React.MouseEvent, text: string | null, type: "steam" | "discord" | "npz") => {
        e.stopPropagation();
        if (!text) return;
    ...
    return (
        ...
        >
            ...
            <td className="py-2.5 border-t border-[var(--border)]">
                <div className="flex items-left justify-left gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                    <button
                        type="button"
                        title={`Скопировать НПЗ: ${npzValue}`}
                        onClick={(e) => handleCopy(e, npzValue, "npz")}
                        className="p-1 rounded hover:bg-[var(--primary)]/20 text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors relative cursor-pointer"
                    >
                        {copiedType === "npz" ? (
