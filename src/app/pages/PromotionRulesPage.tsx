...
const RANK_GROUPS_DEF = [
    ...
    },
    {
        ...
        names: ["Специалист", "Капрал"],
    },
    {
        title: "Сержантский Состав",
        names: ["Сержант", "Штаб-сержант", "Сержант первого класса", "Мастер сержант", "Первый сержант", "Сержант-майор"],
    },
    {
        title: "Старший сержантский состав",
        names: ["Команд сержант-майор", "Сержант-майор сухопутных войск"],
    },
    {
        title: "Младший офицерский Состав",
        names: ["Младший лейтенант", "Лейтенант"],
    },
    {
...
export function PromotionRulesPage() {
    ...
    const rankGroups = RANK_GROUPS_DEF.map((group) => ({
        title: group.title,
        ranks: ranks.filter((r) => group.names.includes(r.name)),
    })).filter((group) => group.ranks.length > 0);

    const skinUrl = selectedRank ? RANK_SKINS[selectedRank.name] : undefined;

    const getRequirementIcon = (description: string) => {
    ...
    if (ranksLoading) {
        return (
            <div className="min-h-screen bg-[#080d17] text-slate-300 font-mono flex items-center justify-center">
                ЗАГРУЗКА ИЕРАРХИИ ЗВАНИЙ...
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-[#080d17] text-slate-300 font-mono py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto space-y-6">
                        ...
                        <div className="absolute top-full left-0 right-0 z-30 mt-1 border border-slate-800 bg-[#080d17] max-h-80 overflow-y-auto divide-y divide-slate-900">
                            {rankGroups.map((group) => (
                                <div key={group.title} className="p-2">
                                    <div className="text-xs font-bold text-[var(--primary)] uppercase px-2 py-1 tracking-widest">
                                        {group.title}
                                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                    <aside className="hidden md:block md:col-span-4 lg:col-span-3 space-y-4 border border-slate-900 bg-slate-950/20 p-4 max-h-[80vh] overflow-y-auto">
                        <div className="text-xl font-bold tracking-widest text-slate-500 uppercase pb-2 border-b border-slate-900">
                            Иерархия званий
                        </div>
                        <div className="space-y-5">
                            {rankGroups.map((group) => (
                                <div key={group.title} className="space-y-1.5">
                                    <h3 className="text-base font-bold text-[var(--primary)] uppercase tracking-wider mb-2">
                                        {group.title}
                                    </h3>
                            ...
                            <div className="lg:col-span-8 space-y-3">
                                {reqsLoading ? (
                                    ...
                                    requirements.map((req, idx) => {
                                        const isFrozen = req.description.toLowerCase().includes("заморожен");
                                        return (
                                            <div 
                                                key={req.id} 
                                            ...
                                            >
                                                <div className="shrink-0 p-2 border border-slate-800 bg-[#080d17] mt-0.5">
                                                    {isFrozen ? <Lock size={16} className="text-red-500" /> : getRequirementIcon(req.description)}
                                                </div>
