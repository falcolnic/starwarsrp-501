...
export function Positions() {
    ...
    useEffect(() => {
        fetch("/api/content/tab_positions")
            .then((res) => res.json())
            .then((data) => {
                try {
                    if (data?.content) {
                    ...
                    console.error("Ошибка парсинга JSON должностей:", e);
                }
                setLoading(false);
            })
    ...
    }, [loading, location.hash, departments]);
    ...
    if (loading) {
        return (
            <div className="max-w-[1200px] mx-auto px-6 py-10 flex items-center justify-center min-h-[50vh]">
                <div className="font-mono text-sm text-[var(--muted-foreground)]">Загрузка базы данных...</div>
            </div>
        );
    }
